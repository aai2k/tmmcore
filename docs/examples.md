...
`tmmThicknessJacobian` returns R and $\partial R/\partial d_j$ for every layer. Evaluating it at each wavelength gives a signed sensitivity map of the quarter-wave reflector from the angle example, with one row per layer and one column per wavelength.
...
## Group delay, against a closed form

`examples/08-group-delay.mjs`

A slab whose surrounding media share its own index has no interfaces, so nothing reflects and the transmission coefficient is a pure phase. Group delay, GDD and TOD then reduce exactly to bulk propagation, which gives a check on the phase kernel that does not go through another transfer-matrix calculation.
...
The example also builds the index jet the way you would in your own code, by composing a Cauchy formula through the jet arithmetic rather than differentiating it by hand.

The second half reads GDD off a chirped mirror and ranks its layers by $\partial\mathrm{GDD}/\partial d$, which arrives in the same call:

```
24-layer chirped mirror, normal incidence

   λ (nm)      R        GD (fs)   GDD (fs²)   TOD (fs³)
     700  0.7330       2.03       87.0       -2839
     750  0.9708       1.80       11.7         -80
     800  0.9994       1.71       -0.3          31
     850  0.9999       2.04       -5.0          52
     900  0.9997       3.35      -21.0         307
```

Negative GDD across the design band is the point of a chirped mirror: the layer thicknesses ramp along the stack, so red turns around deeper than blue and comes back later, which is what compresses a pulse.
